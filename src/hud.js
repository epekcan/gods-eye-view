...
/**
 * @module hud
 * @description TARGIL TÜRKİYE HUD Katmanı — Coğrafi Veri & Durum Farkındalığı Arayüzü.
 */
...
/** HUD renk paletleri */
const HUD_COLORS = {
  surveillance: { main: 'rgba(255, 42, 68, 0.9)',   glow: 'rgba(255, 42, 68, 0.6)',   border: 'rgba(255, 42, 68, 0.3)' },
  thermal:      { main: 'rgba(255, 255, 255, 0.85)', glow: 'rgba(255, 255, 255, 0.5)', border: 'rgba(255, 255, 255, 0.2)' },
  retro:        { main: 'rgba(255, 60, 60, 0.9)',    glow: 'rgba(255, 60, 60, 0.6)',    border: 'rgba(255, 60, 60, 0.3)' },
  _default:     { main: 'rgba(255, 42, 68, 0.85)',  glow: 'rgba(255, 42, 68, 0.5)',  border: 'rgba(255, 42, 68, 0.25)' },
};

/** Shader modları */
const MILITARY_STYLES = new Set(['retro', 'surveillance', 'thermal']);

/** HUD düzen varyantları */
const HUD_VARIANTS = new Set(['tactical', 'operator', 'minimal']);
const HUD_SUMMARY_INTERVAL_MS = 15000;

const HUD_GEOID_CELL_DEG = 0.01;

/** Yakındaki POI listesi */
...
export class IntelHUD {
  constructor(viewer, { placeSearch, summaryPolicy = {}, basemapContext = {} } = {}) {
    ...
    // Askeri kodlar yerine jeo-uzamsal motor ve projeksiyon referansları
    this._missionId = 'CESIUM-3D';
    this._sensorId = 'EPSG:4326';
  ...
  _buildDOM() {
    ...
    this._el.innerHTML = `
      <div class="hud-top-bar">
        <span class="hud-top-bar-left">CANLI // COĞRAFİ VERİ AKIŞI</span>
        <span class="hud-top-bar-center">${this._missionId}</span>
        <span class="hud-top-bar-right">SAYFA 1/1</span>
      </div>

      <div class="hud-corner hud-top-left">
        <div class="hud-bracket">┌</div>
        <div class="hud-content">
          <div class="hud-classification">DURUM // FARKINDALIĞI</div>
          <div class="hud-system">${this._missionId} // ${this._sensorId}</div>
          <div class="hud-mode" id="hud-mode">STANDART</div>
          <div class="hud-summary-wrap">
            <div class="hud-summary-label">ÖZET</div>
            <div class="hud-summary" id="hud-summary">Telemetri bekleniyor...</div>
          </div>
        </div>
      </div>
      ...
      <div class="hud-corner hud-top-right">
        <div class="hud-content" style="text-align:right">
          <div class="hud-rec"><span id="hud-rec-dot">●</span> CANLI  <span id="hud-timestamp">2026-01-01 00:00:00Z</span></div>
          <div class="hud-orbital">YÖRÜNGE: ${this._orbitNum}  GEÇİŞ: ALÇ-${this._passNum}</div>
        </div>
        <div class="hud-bracket">┐</div>
      </div>
      ...
      <div class="hud-corner hud-bottom-right">
        <div class="hud-content" style="text-align:right">
          ...
          <div id="hud-alt">İRT: --m   GÜNEŞ: --° AÇI</div>
      ...
      <div class="hud-bottom-bar">
        <span id="hud-bottom-line">ENL: --  BOY: --  MGRS: ---</span>
      </div>
    `;
  ...
  _updateCameraData() {
    ...
    if (bottomEl) {
      bottomEl.textContent = `MGRS: ${mgrsLabel}  ENL: ${latDMS}  BOY: ${lonDMS}`;
    }
    ...
    if (altEl) altEl.textContent = `İRT: ${Math.round(altMslM)}m   GÜNEŞ: ${sunEl.toFixed(1)}° AÇI`;
    ...
    if (collEl) {
      ...
      const s = String(now.getUTCSeconds()).padStart(2, '0');
      collEl.textContent = `VERİ: ${h}:${m}:${s}Z`;
    }
  ...
  _toDMS(decimal, type) {
    ...
    let dir;
    if (type === 'lat') dir = decimal >= 0 ? 'K' : 'G';
    else dir = decimal >= 0 ? 'D' : 'B';
  ...
  _viewBand(altM) {
    if (altM < 1200) return 'SOKAK';
    if (altM < 5000) return 'ŞEHİR';
    if (altM < 30000) return 'METROPOL';
    if (altM < 250000) return 'BÖLGESEL';
    return 'KÜRESEL';
  }

  _regionLabel(lat, lon) {
    if (lat > 72) return 'KUZEY KUTBU';
    if (lat < -60) return 'GÜNEY KUTBU';
    if (lat >= 5 && lat <= 83 && lon >= -170 && lon <= -50) return 'KUZEY AMERİKA';
    if (lat >= -60 && lat <= 15 && lon >= -90 && lon <= -30) return 'GÜNEY AMERİKA';
    if (lat >= 34 && lat <= 72 && lon >= -25 && lon <= 45) return 'AVRUPA / AKDENİZ';
    if (lat >= -35 && lat <= 38 && lon >= -20 && lon <= 55) return 'AFRİKA';
    if (lat >= 5 && lat <= 80 && lon >= 45 && lon <= 180) return 'ASYA';
    if (lat >= -50 && lat <= 5 && lon >= 110 && lon <= 180) return 'OKYANUSYA';
    return lat >= 0 ? 'KUZEY OKYANUS AĞI' : 'GÜNEY OKYANUS AĞI';
  }
  ...
  _composeSummary() {
    ...
    if (!m) return 'Telemetri bekleniyor...';

    const modeEl = document.getElementById('hud-mode');
    const modeLabel = modeEl?.textContent || 'STANDART';
    ...
    return `${modeLabel} ${band} ${localityTag} | ${region} | İRTİFA ${altTag} | ALAN ${winTag} | GÜNEŞ ${m.sunEl.toFixed(0)}° | ONA ${m.ona.toFixed(0)}° | ${localTag}`;
  }
  ...
  onStyleChange(styleName) {
    ...
    const modeEl = document.getElementById('hud-mode');
    if (modeEl) {
      const modeNames = { surveillance: 'NVG', thermal: 'FLIR', retro: 'CRT', normal: 'STANDART' };
